...
<script>
    import {requestServiceDetail} from '../../api/axiosapi';

    import {requestSubmitEditService} from '../../api/axiosapi';
  export default {
    data() {
      return {
        form: {
    ...
    methods: {
        getData(){
            ...
            requestServiceDetail(params).then(result => {
                if (result.code !== 200) {
                    this.$message({
                        message: result.msg,
                        type: 'error'
        ...
        onSubmit(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    requestSubmitEditService(params).then(result => {
                        if (result.code !== 200) {
                            this.$message({
                                message: result.msg,
                                type: 'error'
